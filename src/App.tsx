...
interface Property { name?: string; label?: string; type?: string; object?: string; }
...
function App() {
  ...
  useEffect(() => {
    async function getNativeProperties() {
      ...
      const nativePropertyOptions = properties.map((property: Property) => {
        return { name: property.name, label: property.label, type: property.type, object: property.object }
      })
  ...
  const renderContactProperties = () => {
    const contactProperties = nativeProperties.length > 0 ? nativeProperties.filter(property => {
      return property.object === "Contact"
    }) : null

    console.log(contactProperties)

    const contactPropertiesMappings = contactProperties ? contactProperties.map(property => {
      return <MappingDisplay hubspotProperties={hubspotContactProperties} nativeProperty={property} setMappings={setMappings} mappings={mappings} objectType="Contact" />
    }) : null
    console.log(contactPropertiesMappings)
    return (<div className='contact-property-mappings-wrapper'><> {contactPropertiesMappings} </></div>)
  }


  const renderCompanyProperties = () => {
    const companyProperties = nativeProperties.length > 0 ? nativeProperties.filter(property => {
      return property.object === "Company"
    }) : null

    console.log(companyProperties)

    const companyPropertiesMappings = companyProperties ? companyProperties.map(property => {
      return <MappingDisplay hubspotProperties={hubspotCompanyProperties} nativeProperty={property} setMappings={setMappings} mappings={mappings} objectType="Company" />
    }) : null
    console.log(companyPropertiesMappings)
    return (<div className='contact-property-mappings-wrapper'><> {companyPropertiesMappings} </></div>)
  }
    ...
    <Box sx={{ flexGrow: 1 }}>
      <Grid container spacing={12} columns={12} >
        ...
        <Grid id="bodyContainer" xs={8} item>
          ...
          <BasicTabs objects={["Contact", "Company"]} tabContent={[renderContactProperties(), renderCompanyProperties()]} />
